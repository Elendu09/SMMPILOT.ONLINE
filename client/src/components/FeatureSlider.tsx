      ...
      <div className="overflow-hidden w-full" ref={emblaRef}>
        <div className="flex touch-pan-y">
          {features.map((feature, index) => (
            ...
            >
              <div className="flex flex-col items-center text-center space-y-2 md:space-y-3 p-4 md:p-6 rounded-2xl bg-zinc-50/50 border border-zinc-100/50 shadow-sm">
                <div className="p-3 bg-white rounded-xl shadow-sm text-zinc-900 border border-zinc-100">
                  {feature.icon}
                </div>
                <h3 className="text-lg md:text-xl font-bold text-zinc-900 font-display">
                  {feature.title}
                </h3>
                <p className="text-sm md:text-base text-zinc-500 max-w-[280px] md:max-w-md mx-auto leading-relaxed">
                  {feature.description}
                </p>
              </div>
            </div>
        ...
        {features.map((_, index) => (
          <button
            ...
            className={`transition-all duration-300 ease-out rounded-full ${
              index === selectedIndex 
                ? "w-6 md:w-8 h-1.5 md:h-2 bg-zinc-900" 
                : "w-1.5 md:w-2 h-1.5 md:h-2 bg-zinc-300 hover:bg-zinc-400"
            }`}
